"""Go code review reporter.

Uses the generic line-based analyzer for initial estimates.
Full analysis requires go vet / staticcheck.
"""
...
def analyze_files(
    ...
    """Analyze Go source files for code quality issues."""
    report = ReviewReport(
    ...
    if not report.findings:
        report.add_finding(
            Finding(
                ...
                message=(
                    "Go analysis is line-based estimation. "
                    "For accurate results, run go vet / staticcheck."
                ),
                suggestion="Consider integrating go vet into CI.",
            )
        )
