"""Rust code review reporter.

Uses the generic line-based analyzer for initial estimates.
Full analysis requires clippy / cargo check.
"""
...
def analyze_files(
    ...
    """Analyze Rust source files for code quality issues."""
    report = ReviewReport(
    ...
    if not report.findings:
        report.add_finding(
            Finding(
                ...
                message=(
                    "Rust analysis is line-based estimation. "
                    "For accurate results, run cargo clippy."
                ),
                suggestion="Consider integrating clippy into CI.",
            )
        )
